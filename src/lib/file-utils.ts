...
export async function downloadEntityFile(
  ...
  // Use SDK's built-in method for file download
  // Note: This downloads the default content. For specific keys,
  // implement custom fetch with query parameter if needed.
  const { data, error } = await client.getFileContentAsArrayBuffer(entityId);

  if (error || !data) {
    throw new Error(`DOWNLOAD_FAILED: Failed to download from ${entityId}: ${JSON.stringify(error)}`);
  }
